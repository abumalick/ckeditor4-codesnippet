...
(function() {
  const isBrowserSupported = !CKEDITOR.env.ie || CKEDITOR.env.version > 8;
  ...
  CKEDITOR.plugins.add('codesnippet', {
    ...
    beforeInit(editor) {
      ...
      this.setHighlighter = function(highlighter) {
        ...
        let langs = (editor._.codesnippet.langs =
        ...
        editor._.codesnippet.langsRegex = new RegExp(
          `(?:^|\\s)language-(${ 
            CKEDITOR.tools.objectKeys(langs).join('|') 
            })(?:\\s|$)`,
        );
      };
    },
    ...
    afterInit(editor) {
      ...
      if (!editor._.codesnippet.highlighter) {
        let hljsHighlighter = new CKEDITOR.plugins.codesnippet.highlighter({
          languages: {
            apache: 'Apache',
            bash: 'Bash',
  ...
  function registerWidget(editor) {
    const codeClass = editor.config.codeSnippet_codeClass;

    let newLineRegex = /\r?\n/g;

    let textarea = new CKEDITOR.dom.element('textarea');

    let lang = editor.lang.codesnippet;
    ...
    editor.widgets.add('codeSnippet', {
      ...
      template: `<pre dir="ltr"><code class="${codeClass}"></code></pre>`,
      dialog: 'codeSnippet',
...
var callback = function(formatted) {
...
var oldData = this.oldData;
      ...
      upcast(el, data) {
        ...
        let childrenArray = getNonEmptyChildren(el);
...
var code;
        ...
        let matchResult = editor._.codesnippet.langsRegex.exec(
    ...
    function getNonEmptyChildren(parentElement) {
      ...
      let preChildrenList = parentElement.children;

      let curNode;

      // Filter out empty text nodes.
      for (let i = preChildrenList.length - 1; i >= 0; i--) {
